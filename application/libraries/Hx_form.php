...
class Hx_form {
   ...
   public function set_template($arr,$arr_field,$values=array())
   {
      ...
      $js_form = (isset($arr['js_form'])) ? $arr['js_form'] : null;
      ...
      $form  = '<div class="modal-dialog '.$arr['cs_modal'].'">
                ...
                <form id="form_input" action="'.site_url($arr['url_action']).'" method="post" class="form-'.$arr['cs_form'].'" '.$attr.'>
   ...
   public function set_input($layout,$name,$label,$value=null,$type,$list=null,$lebar=null,$attr=null)
   {
      ...
      switch ($type):
         ...
         case 'select':
            ...
            $input .= $buka_div;
            $input .= '<select name="data['.$name.']" id="'.$name.'" class="form-control select_two" '.$attr.'>';
		 ...
		 case 'select2':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<select name="data['.$name.']" id="tahun1" class="form-control select_two" '.$attr.'>';
            $input .= '   <option value="">- Pilih '.$label.' -</option>';
         ...
         case 'radio':
            ...
            foreach ($list as $_index=>$_item):
               ...
               $input .= '  <input type="radio" name="data['.$name.']" id="'.$name.$_index.'" value="'.$_index.'" '.$_checked.' '.$attr.'><label for="'.$name.$_index.'">'.$_item.'</label>';
               $input .= '</div>';
            endforeach;
         ...
         case 'checkbox':
            ...
            foreach ($list as $_index=>$_item):
               $_checked = (array_search($_index,$value)===FALSE) ? '' : 'checked';
               $input .= '<div class="checkbox checkbox-success checkbox-inline">';
               $input .= '  <input type="checkbox" name="data['.$name.'][]" id="'.$name.$_index.'" value="'.$_index.'" '.$_checked.'><label for="'.$name.$_index.'">'.$_item.'</label>';
               $input .= '</div>';
            endforeach;
         ...
         case 'textarea':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<textarea name="data['.$name.']" id="'.$name.'" class="form-control" '.$attr.'>'.$value.'</textarea>';
            $input .= $tutup_div;
            $input .= '</div>';
         break;
         ...
         case 'editor':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<textarea name="data['.$name.']" id="'.$name.'" class="form-control editor-full" rows="8" '.$attr.'>'.$value.'</textarea>';
            $input .= $tutup_div;
            $input .= '</div>';
         break;
         ...
         case 'editor-mini':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<textarea name="data['.$name.']" id="'.$name.'" class="form-control editor-mini" rows="8" '.$attr.'>'.$value.'</textarea>';
            $input .= $tutup_div;
            $input .= '</div>';
         break;
         ...
         case 'number':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<input type="number" name="data['.$name.']" id="'.$name.'" value="'.$value.'" class="form-control num" '.$attr.'>';
            $input .= $tutup_div;
            $input .= '</div>';
         break;
         ...
         case 'numeric':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<input type="text" name="data['.$name.']" id="'.$name.'" value="'.$value.'" class="form-control num" '.$attr.'>';
            $input .= $tutup_div;
            $input .= '</div>';
         break;
         ...
         case 'rupiah':
            ...
            $input .= '  <input type="text" name="data['.$name.']" id="'.$name.'" value="'.$value.'" class="form-control rupiah num" '.$attr.' data-a-sep="." data-a-dec=",">';
            $input .= '</div>';
            $input .= $tutup_div;
            $input .= '</div>';
         break;
         ...
         case 'tanggal':
            ...
            $input .= '  <input type="text" name="data['.$name.']" id="'.$name.'" value="'.$value.'" class="form-control tgl" '.$attr.'>';
         ...
         case 'time':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<div class="bootstrap-timepicker">';
            $input .= '<div class="input-group">';
            $input .= '<input type="time" name="data['.$name.']" id="'.$name.'" value="'.$value.'" class="form-control jam" '.$attr.'>';
         ...
         case 'password':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<input type="'.$type.'" name="data['.$name.']" id="'.$name.'" class="form-control" '.$attr.'>';
         ...
         //------->jika type hidden
         case 'hidden':
            $input = '<input type="hidden" name="data['.$name.']" id="'.$name.'" value="'.$value.'">';
         break;
         ...
         case 'panjang':
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<div id="div-panjang"><input type="text" name="data['.$name.']" id="'.$name.'" value="'.$value.'" class="form-control" '.$attr.'></div>';
            $input .= $tutup_div;
            $input .= '</div>';
         break;
         ...
         case 'panjang-benih':
            ...
            $input .= '<select name="data['.$name.']" id="'.$name.'" class="form-control" '.$attr.'>';
            $input .= '   <option value="">- Pilih '.$label.' -</option>';
         ...
         default:
            ...
            $input .= '<label for="'.$name.'" class="'.$class_label.' control-label">'.$label.'</label>';
            $input .= $buka_div;
            $input .= '<input type="'.$type.'" name="data['.$name.']" id="'.$name.'" value="'.$value.'" class="form-control" '.$attr.'>';
            $input .= $tutup_div;
            $input .= '</div>';
         break;
